...
- **Slash commands** — defined server-side (`src/commands/registry.ts`) so the TUI and the web dashboard share one implementation. Typing `/` opens a filtered menu above the prompt with a description per command, so nothing has to be memorised: `/help /status /agents /tasks /model /sessions /resume /clear /cancel /undo /cost /mcp /lsp /permissions /init /graph /usage`, plus `/theme`, `/dashboard`, and `/quit` (client-side), plus your own in `.amux/commands/<name>.md` (frontmatter + a prompt body, `$ARGUMENTS` interpolated).
