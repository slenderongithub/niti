...
export async function serveMain(opts: { port?: number; interactive?: boolean } = {}): Promise<ServeResult> {
...
if (import.meta.main) {
  ...
  serveMain({ port: portArg ? Number(portArg.slice(7)) : undefined }).catch((err) => {
